...
/**
 * Simple local audio player. Pass a playlist of local files:
 * [{ title, artist, src, cover? }]
 * Files should live in /public/audio/*.mp3 (or .ogg/.wav)
 */
export default function MusicPlayer({
  ...
  // Load & optionally autoplay on index change
  useEffect(() => {
    if (!audioRef.current) return
    setCurrentTime(0)
    setDuration(0)
    ...
    const playIfNeeded = async () => {
      if (autoPlay || isPlaying) {
        try {
          ...
          // Autoplay will be blocked until user gesture — ignore
          setIsPlaying(false)
        }
      }
    }
  ...
  const onPrev = () => {
    ...
    if (isShuffle) {
      setIndex(randomIndex(playlist.length, index))
    } else {
  ...
  return (
    <div className={`pointer-events-auto select-none rounded-xl bg-black/40 border border-white/10 backdrop-blur p-3 ${className}`}>
      ...
      <div className="mt-3 flex items-center gap-2">
        <button
          className={`p-2 rounded hover:bg-white/10 ${isShuffle ? 'text-cyan-300' : 'text-white/80'}`}
          title="Shuffle"
        ...
        <button className="p-2 rounded hover:bg-white/10 text-white/90" onClick={onPrev} title="Previous">
          <SkipBack size={18} />
        </button>

        <button
          className="p-2 rounded bg-white/15 hover:bg-white/25 text-white"
          onClick={onPlayPause}
        ...
        <button className="p-2 rounded hover:bg-white/10 text-white/90" onClick={onNext} title="Next">
          <SkipForward size={18} />
        </button>

        <button
          className={`p-2 rounded hover:bg-white/10 ${isLoop ? 'text-cyan-300' : 'text-white/80'}`}
          title="Repeat"
